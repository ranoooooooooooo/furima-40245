  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規作成できない場合' do
      ...
      it 'passwordで半角英字のみでは登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("Password パスワードは半角英数字のみ、半角英数字混合で設定してください")
      end
      ...
      it 'お名前(全角)は、名字を全角（漢字・ひらがな・カタカナ）で入力しないと登録できない' do
        @user.last_name = 'ﾔﾏﾀﾞ'
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name お名前は全角文字で入力してください")
      end

      it 'お名前(全角)は、名前を全角（漢字・ひらがな・カタカナ）で入力しないと登録できない' do
        @user.first_name = 'ﾀﾛｳ'
        @user.valid?
        expect(@user.errors.full_messages).to include("First name お名前は全角文字で入力してください")
      end
      ...
      it 'お名前カナ(全角)は、名字を全角（カタカナ）で入力しないと登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana フリガナは全角カタカナのみで入力してください")
      end

      it 'お名前カナ(全角)は、名前を全角（カタカナ）で入力しないと登録できない' do
        @user.first_name_kana = 'ﾀﾛｳ'
        @user.valid?
        expect(@user.errors.full_messages).to include("First name kana フリガナは全角カタカナのみで入力してください")
      end
